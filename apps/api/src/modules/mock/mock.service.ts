...
const mapExamSummary = (exam: any) => {
  ...
  return {
    ...
    canAttempt: !attempt || attempt.status === AttemptStatus.IN_PROGRESS,
    isAttempted: Boolean(attempt && isSubmittedAttempt(attempt.status)),
  };
};
...
export const listExams = async (studentId: string, examTypeId: string, mockExamTypeId: string) => {
  const exams = await prisma.mockExam.findMany({
    where: { isActive: true, examTypeId, mockExamTypeId, ...access(studentId) },
...
export const getExamDetail = async (studentId: string, examId: string) => {
  const exam = await prisma.mockExam.findFirst({
    where: { id: examId, isActive: true, ...access(studentId) },
    include: {
      difficulty: true,
      examType: { select: { id: true, name: true } },
      mockExamType: { select: { id: true, name: true } },
...
export const getCatalog = async (studentId: string, input: any) => {
  ...
  const [examTypes, examTypesWithCounts, exams] = await Promise.all([
    ...
    prisma.mockExam.findMany({ where, orderBy: input.sort === 'marks' ? { totalMarks: 'desc' } : input.sort === 'duration' ? { durationMinutes: 'asc' } : { createdAt: 'desc' }, include: { difficulty: true, examType: true, mockExamType: true, analytics: true, _count: { select: { sections: true, attempts: { where: { status: { in: submitted } } } } }, attempts: { where: { studentId }, take: 1, orderBy: { createdAt: 'desc' }, select: { id: true, status: true, submittedAt: true } }, sections: { select: { _count: { select: { questions: { where: { isActive: true } } } } } } } }),
  ]);
  return { examTypes, mockExamTypes: examTypesWithCounts, exams: exams.filter((x) => input.attempted === undefined || (input.attempted === 'true') === Boolean(x.attempts.length)).map((x) => ({ ...x, totalQuestions: x.sections.reduce((sum, section) => sum + section._count.questions, 0), averageScore: n(x.analytics?.averageScore), attempted: x.attempts[0] ?? null })) };
};
...
export const startAttempt = async (studentId: string, examId: string) => prisma.$transaction(async (tx) => {
  ...
  return tx.mockAttempt.create({ data: { studentId, mockExamId: exam.id, startedAt: new Date(), status: AttemptStatus.IN_PROGRESS, totalMarks: exam.totalMarks, sections: { create: exam.sections.map((s) => ({ mockSectionId: s.id })) }, answers: { create: exam.sections.flatMap((s) => s.questions.map((q, index) => ({ mockQuestionId: q.id, mockSectionId: s.id, selectedAnswers: [], correctAnswers: q.correctAnswers, answerOrder: s.sequenceNumber * 10000 + index + 1 }))) } }, select: { id: true } });
});

const ownedAttempt = async (studentId: string, id: string) => { const attempt = await prisma.mockAttempt.findFirst({ where: { id, studentId }, include: { mockExam: { include: { sections: { orderBy: { sequenceNumber: 'asc' }, include: { mockSectionType: true, questions: { where: { isActive: true }, orderBy: { sequenceNumber: 'asc' }, include: { topic: { include: { subject: true } }, subtopic: true, difficulty: true, mockComprehension: true } } } } }, difficulty: true, examType: true, mockExamType: true }, answers: true, sections: true } }); if (!attempt) throw new AppError(404, 'Attempt not found.'); return attempt; };
...
export const saveAnswer = async (studentId: string, attemptId: string, questionId: string, data: any) => { const attempt = await ownedAttempt(studentId, attemptId); if (attempt.status !== AttemptStatus.IN_PROGRESS) throw new AppError(409, 'Submitted attempts cannot be changed.'); const answer = attempt.answers.find((x) => x.mockQuestionId === questionId); const question = attempt.mockExam.sections.flatMap((x) => x.questions).find((x) => x.id === questionId); if (!answer || !question) throw new AppError(404, 'Question not found.'); const section = attempt.mockExam.sections.find((x) => x.id === question.mockSectionId)!; if (data.visited && !section.canGoBackToPreviousQuestion) { const later = attempt.answers.some((x) => x.mockSectionId === section.id && x.visited && x.answerOrder > answer.answerOrder); if (later) throw new AppError(409, 'You cannot return to an earlier question in this section.'); }
...
export const submitAttempt = async (studentId: string, id: string, sectionTimes: any[]) => prisma.$transaction(async (tx) => { const attempt = await ownedAttempt(studentId, id); if (attempt.status !== AttemptStatus.IN_PROGRESS) throw new AppError(409, 'This attempt has already been submitted.'); const now = new Date(); const elapsed = Math.min(attempt.mockExam.durationMinutes * 60, Math.max(0, Math.floor((now.getTime() - attempt.startedAt.getTime()) / 1000))); let correct = 0, incorrect = 0, unattempted = 0, score = 0; const sectionStats = new Map<string, any>(); for (const sec of attempt.mockExam.sections) sectionStats.set(sec.id, { correct: 0, incorrect: 0, unattempted: 0, score: 0 });
  for (const answer of attempt.answers) { const q = attempt.mockExam.sections.flatMap((s) => s.questions).find((x) => x.id === answer.mockQuestionId)!; const selected = answer.selectedAnswers as string[]; let status = AnswerStatus.UNATTEMPTED, marks = 0; if (!selected.length) unattempted++; else if (arraysEqual(selected, q.correctAnswers)) { status = AnswerStatus.CORRECT; marks = n(q.positiveMarks); correct++; } else { status = q.questionType === 'MULTIPLE_CORRECT' && selected.some((x) => (q.correctAnswers as string[]).includes(x)) ? AnswerStatus.PARTIALLY_CORRECT : AnswerStatus.INCORRECT; marks = -n(q.negativeMarks); incorrect++; } score += marks; const s = sectionStats.get(q.mockSectionId); if (status === AnswerStatus.CORRECT) s.correct++; else if (status === AnswerStatus.UNATTEMPTED) s.unattempted++; else s.incorrect++; s.score += marks; await tx.mockAttemptAnswer.update({ where: { id: answer.id }, data: { status, marksAwarded: marks } }); }
