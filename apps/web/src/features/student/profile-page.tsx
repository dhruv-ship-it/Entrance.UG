...
import { Camera, CheckCircle2, CircleUserRound, LoaderCircle, Mail, Save, ShieldCheck } from 'lucide-react';
...
export const ProfilePage = () => {
  ...
  const requestChangeOtp = useMutation({
    ...
    onSuccess: async (data) => {
      setDevOtp(data.verification.devOtp);
      setEmailMessage('New email saved as not verified. Enter the OTP to verify it.');
      await client.invalidateQueries({ queryKey: ['student-profile'] });
  ...
  const emailChanged = emailForm.email.trim().toLowerCase() !== profile.data.email.toLowerCase();
          ...
          <div className="p-6">
            ...
            <div className="mt-6 rounded-2xl bg-moss-50 p-4">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold">Email {profile.data.emailVerified ? 'verified' : 'not verified'}</p>
                  <p className="mt-1 text-xs leading-5 text-stone-500">{profile.data.emailVerified ? 'Verified email is active for account communication.' : 'Verify your email before production communication is enabled.'}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6 sm:p-8">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-stone-500">Change or verify your email using a one-time code.</p>
                </div>
              </div>
              <Badge className={profile.data.emailVerified ? 'bg-emerald-100 text-emerald-800' : 'bg-amber/20 text-[#93620c]'}><ShieldCheck size={13} />{profile.data.emailVerified ? 'Verified' : 'Not verified'}</Badge>
            </div>
            <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_auto]">
              <Input type="email" value={emailForm.email} onChange={(event) => setEmailForm((current) => ({ ...current, email: event.target.value.toLowerCase() }))} />
              <Button type="button" variant={emailChanged ? 'primary' : 'outline'} disabled={requestCurrentOtp.isPending || requestChangeOtp.isPending || !emailForm.email.includes('@')} onClick={() => emailChanged ? requestChangeOtp.mutate() : requestCurrentOtp.mutate()}>
                {emailChanged ? 'Save & send OTP' : 'Send OTP'}
              </Button>
            </div>
            <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto]">
              ...
              <Button type="button" disabled={!canVerify || verifyCurrentOtp.isPending || verifyChangeOtp.isPending} onClick={() => emailChanged ? verifyChangeOtp.mutate() : verifyCurrentOtp.mutate()}>Verify email</Button>
            </div>
            ...
            {(requestCurrentOtp.isError || requestChangeOtp.isError || verifyCurrentOtp.isError || verifyChangeOtp.isError) && (
              <p className="mt-3 rounded-xl bg-red-50 px-3 py-2.5 text-sm text-red-700">
                {[requestCurrentOtp.error, requestChangeOtp.error, verifyCurrentOtp.error, verifyChangeOtp.error].find(Boolean) instanceof Error
                  ? ([requestCurrentOtp.error, requestChangeOtp.error, verifyCurrentOtp.error, verifyChangeOtp.error].find(Boolean) as Error).message
